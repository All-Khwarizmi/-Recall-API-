...
if (!parsedRequestData.success) return res.status(400).json({msg: 'Please be sure to fill the body of your request with valid data. Refer API documentation.'})
console.log("parsedRequestData", parsedRequestData);
  ...
  try {
    ...
    console.log(error)
    res.status(500).json({msg: 'We could not create the new recall plan', error})
  }
